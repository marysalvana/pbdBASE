...
#' pbdMPI::execmpi(spmd.code = spmd.code, nranks = 2L)
#' 
#' @useDynLib pbdBASE R_CROSSCOVSUBMAT
#' @rdname lclgblmat
#' @export
base.crosscovsubmat <- function(model, param, x, descx)
{
  ldim <- base.numroc(dim=descx[3L:4L], bldim=descx[5L:6L], ICTXT=descx[2L], fixme=TRUE)
  ...
  subx <- .Call(R_CROSSCOVSUBMAT, model, param, x, as.integer(ldim), as.integer(descx))
  subx
}
